...
module AssocListMap : Map = struct
  (** The list [(k1, v1); ...; (kn, vn)] binds key
      [ki] to value [vi]. If a key appeaers more
      than once in the list, it is bound to the
      left-most occurence in the list. *)
  type ('k, 'v) t = ('k * 'v) list
  ...
  let bindings m =
    m
    |> keys
    |> List.map (fun k -> (k, lookup k m))
end

module UniqAssocListMap : Map = struct
    (** The list [(k1, v1); ...; (kn, vn)] binds
        key [ki] to value [vi]. No duplicate keys
        may occur. *)
  type ('k, 'v) t = ('k * 'v) list
